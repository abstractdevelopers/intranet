...
- Promise Wall: the pre-class creator wall — one line per student in `Promise` (unique `userId`) saying what they intend to build, plus a picked `goal` (keys in `PROMISE_GOALS`) and a `pathway` craft derived server-side from their elective (never from the client). Peers react with `PromiseCheer` (unique `promiseId`+`userId`). Logic in `src/lib/promises.ts`; UI at `/student/promise-wall` with craft filter chips; API at `/api/student/promises` and `/api/student/promises/[id]/cheer`. Posting closes end of 3 Oct (`PROMISE_WALL_CLOSES`); the wall stays readable after. `CLASSES_START` is 5 Oct. Craft prompts live in `PROMISE_PROMPTS_BY_PATHWAY`. Academy-spirit extras on the same page: the Craft Race (`rankCrafts`, pure — reuses the counts already loaded) and the Cohort Pulse (`getCohortPulse` — creators/promises/cheers/days-to-class).
