...
- Warm-up (the Promise Wall): the pre-class course task — one line per student in `Promise` (unique `userId`) saying what they want to make, plus a picked `goal` (keys in `PROMISE_GOALS`) and a `pathway` craft derived server-side from their elective (never from the client). Peers can cheer with `PromiseCheer` (unique `promiseId`+`userId`). Logic in `src/lib/promises.ts`; UI at `/student/warm-up` (route was `/student/promise-wall`) with quiet course-filter counts; API at `/api/student/promises` and `/api/student/promises/[id]/cheer`. Writing closes end of 3 Oct (`PROMISE_WALL_CLOSES`); lines stay readable after. `CLASSES_START` is 5 Oct. Prompts per course live in `PROMISE_PROMPTS_BY_PATHWAY`. Deliberately no leaderboard/pulse/points — it is a course task, not a game.
